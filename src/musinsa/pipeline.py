...
def main(
    ...
    log_info(f"카테고리: 런타임에 동적 발견 (서브카테고리 단위 크롤)")
    log_info(f"카테고리별 최대 상품 수: {'전체' if max_products_resolved is None else max_products_resolved}")
        ...
        with make_progress() as progress:
            ...
            with ThreadPoolExecutor(max_workers=workers_resolved) as executor:
                futures = {
                    executor.submit(
                        crawl_listing_unit,
                        pool, unit, max_products_resolved, max_scrolls, delay,
                    ): unit
            ...
            if not skip_options and all_products:
                detail_task = progress.add_task(
                    "상세 페이지/옵션 수집", total=len(all_products)
                )
                ...
                with ThreadPoolExecutor(max_workers=workers_resolved) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            continue
                        updated_by_id[updated_product.product_id or updated_product.product_url] = updated_product

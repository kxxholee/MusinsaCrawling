...
무신사 스탠다드 여성 상품(브랜드 `musinsastandard`, 필터 `gf=F`)을 Selenium + BeautifulSoup으로 동적 수집해서 보고서형 `SKU_요약`과 검증용 raw 시트들을 포함한 엑셀을 만드는 단일 파일 데모입니다. Python 3.11, 패키지는 `uv` 관리.

실제 진입점은 [musinsa_sku_generator_demo.py](musinsa_sku_generator_demo.py) 입니다. [main.py](main.py) 는 `uv init` 의 잔재이므로 사용하지 않습니다.
...
uv run python musinsa_sku_generator_demo.py --max-products 10 --headless --skip-options
```

CLI 플래그: `--output`, `--max-products` (0이면 전체), `--max-scrolls`, `--delay` (상세 페이지 사이 대기 시간, 초 단위 — 너무 낮추지 말 것), `--headless`, `--skip-options`.

테스트, 린터, 포매터는 설정되어 있지 않습니다.

## 아키텍처

스크립트는 한 파일 안에 번호 매겨진 섹션으로 선형 파이프라인을 이루고 있습니다. 데이터 흐름은 다음과 같습니다.

```
TARGET_CATEGORIES → collect_products_from_category → ProductRow[]
                                                  → collect_detail_and_options → OptionRow[], SkuRow[]
                                                  → save_excel (openpyxl로 SKU_요약 + raw 시트 생성)
```

스키마와 엑셀 컬럼 순서는 세 개의 dataclass — `ProductRow`, `OptionRow`, `SkuRow` (섹션 3 / 8) 가 정의합니다. 컬럼 리스트는 `__annotations__` 에서 그대로 끌어옵니다.
...
정리·색상/사이즈 추정·중복 제거는 파이썬 단계에서 합니다 (`guess_product_name`, `extract_size_from_text`, `find_color_words`, `parse_options_from_candidates`). 사이트 구조가 깨지면 **dataclass 나 엑셀 레이어가 아니라 위 함수들과 셀렉터부터 손봐야 합니다.**
...
- `CATEGORY_MASTER` 는 대분류별 raw 서브카테고리를 모두 나열하지만, 실제 크롤링은 `TARGET_CATEGORIES` 의 **대분류 코드** (`001` 상의, `003` 바지) 단위로만 돕니다. 상품의 raw 카테고리는 `detect_raw_category` 에서 빵부스러기 / 본문 텍스트로 역추정합니다.
...
`save_excel` 은 시트 순서를 `SKU_요약`, `model_summary`, `products_raw`, `options_raw`, `sku_result`, `category_master` 로 고정합니다. `SKU_요약`은 A열 대분류, B/F/G/H/I열 중분류 단위로 병합하고, raw 시트들은 `adjust_excel_format` 이 첫 행 고정·자동 필터·헤더 스타일·컬럼 폭을 적용합니다. legacy `summary` 시트는 만들지 않습니다.
